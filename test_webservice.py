...
def fail_fn():
    raise Exception("This is a failure function.")
...
def test_fail_fn():
    ...
    for i in range(20):
        ...
        if resp.json()['status'] == "FAILED":
            logging.warning(f"Task is now in {resp.json()['status']}")
            break
        elif resp.json()['status'] == "COMPLETED":
            assert False, "Expected task to fail, but it completed."
...
def test_factorial_fn():
    # Register the factorial function
    resp = requests.post(base_url + "register_function",
                         json={"name": "factorial",
                               "payload": serialize(factorial)})
    fn_info = resp.json()
    assert "function_id" in fn_info

    # Execute the function
    number = random.randint(0, 10)  # keep the number small to avoid large results
    resp = requests.post(base_url + "execute_function",
                         json={"function_id": fn_info['function_id'],
                               "payload": serialize(((number,), {}))})
    ...
    for i in range(20):
        ...
        if resp.json()['status'] in ["COMPLETED", "FAILED"]:
            ...
            result = deserialize(s_result['result'])
            print(f'result:{result}')
            assert result == factorial(number)  # compare with locally computed factorial
            break
